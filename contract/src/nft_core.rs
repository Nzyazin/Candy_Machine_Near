...
const MIN_DEPOSIT: u128 = 1_500_000_000_000_000_000_000_000;
...
impl NonFungibleTokenCore for Contract {
    ...
    fn nft_mint(
        ...
        //verifying enough deposit
        let amount = env::attached_deposit();
        assert!(amount >= MIN_DEPOSIT, "Not enough deposit!");
        //get the sender to transfer the token from the sender to the receiver
        let sender_id = env::predecessor_account_id();
        //get vector of tokens id
        let mut vec_with_token_ids = Self::selling_token(&self, Some(U128(0)), Some(12));
        //removing last element from vector if it exists
        let token_id = match vec_with_token_ids.pop() {
            Some(cur) => cur,
